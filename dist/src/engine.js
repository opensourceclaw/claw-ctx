...
const INFO = { id: "claw-ctx", name: "Claw Context Engine", version: "1.0.0", ownsCompaction: false, turnMaintenanceMode: "foreground", hostRequirements: {} };
class SearchCache {
    store = new Map();
    ttl;
...
class ClawContextEngine {
    ...
    async assemble(p) {
        ...
        const items = mems.filter((m) => (m.score ?? 0) >= 0.3).map((m) => ({ content: m.content ?? "", score: m.score ?? 0 }));
